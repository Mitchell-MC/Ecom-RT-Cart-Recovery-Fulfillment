...
def build_zorder_copy(spark: SparkSession, source_table: str, target_table: str) -> None:
...
def sample_lookup_predicate(spark: SparkSession, source_table: str) -> tuple[str, str, str]:
...
def main():
    ...
    results = {}
    for label, table in [("A: partition-only", layout_a), ("B: partition + ZORDER", layout_b)]:
        durations = [time_query(spark, table, cart_id, start_date) for _ in range(N_RUNS)]
        results[label] = {
    ...
    for label, r in results.items():
        print(f"{label:<24} {r['avg_seconds']:>10.3f} {r['min_seconds']:>10.3f} {r['num_files']:>10}")
