...
def aggregate_carts(events: DataFrame) -> DataFrame:
    ...
    return cart_events.groupBy("cart_id").agg(
        ...
        F.max(F.when(F.col("event_type") == "add_to_cart", 1).otherwise(0)).alias("has_add"),
        F.max(F.when(F.col("event_type") == "checkout_complete", 1).otherwise(0))
            .alias("has_checkout_complete"),
        F.sum(F.when(F.col("event_type") == "add_to_cart",
                      F.col("price_at_event") * F.col("quantity")).otherwise(0.0))
            .alias("added_value"),
        F.sum(F.when(F.col("event_type") == "remove_from_cart",
                      F.col("price_at_event") * F.col("quantity")).otherwise(0.0))
            .alias("removed_value"),
        F.sum(F.when(F.col("event_type") == "add_to_cart", 1).otherwise(0)).alias("n_adds"),
        F.sum(F.when(F.col("event_type") == "remove_from_cart", 1).otherwise(0)).alias("n_removes"),
    )
...
def filter_abandoned(cart_agg: DataFrame) -> DataFrame:
    return (
        cart_agg
        .filter(F.col("has_add") == 1)
        .filter(F.col("has_checkout_complete") == 0)
        .filter(F.col("last_activity_at") <
                F.expr(f"current_timestamp() - INTERVAL {CART_ABANDON_THRESHOLD_MINUTES} MINUTES"))
        .filter(F.col("last_activity_at") >
                F.expr(f"current_timestamp() - INTERVAL {CART_STALE_HORIZON_DAYS} DAYS"))
        .withColumn("recoverable_cart_value",
                    F.greatest(F.col("added_value") - F.col("removed_value"), F.lit(0.0)))
        .withColumn("item_count", F.greatest(F.col("n_adds") - F.col("n_removes"), F.lit(0)))
        .withColumn("minutes_since_last_activity",
                    (F.unix_timestamp(F.current_timestamp()) - F.unix_timestamp("last_activity_at")) / 60.0)
    )
...
def score_carts(abandoned: DataFrame, orders: DataFrame) -> DataFrame:
    ...
    return (
        abandoned
        .join(prior_orders, on="customer_id", how="left")
        .withColumn("has_prior_order", F.coalesce(F.col("prior_order_count"), F.lit(0)) > 0)
        .withColumn("recency_score", 100 * F.exp(-F.col("minutes_since_last_activity") / 180.0))
        .withColumn("value_score",
                    100 * F.least(F.col("recoverable_cart_value") / F.lit(value_p90), F.lit(1.0)))
        .withColumn("loyalty_score", F.when(F.col("has_prior_order"), 100.0).otherwise(40.0))
        .withColumn("priority_score",
                    0.45 * F.col("recency_score") + 0.40 * F.col("value_score")
                    + 0.15 * F.col("loyalty_score"))
        .withColumn("_gold_computed_at", F.current_timestamp())
        .select("cart_id", "customer_id", "last_activity_at", "minutes_since_last_activity",
                "recoverable_cart_value", "item_count", "recency_score", "value_score",
                "loyalty_score", "priority_score", "_gold_computed_at")
    )
...
def main():
    ...
    scored.write.format("delta").mode("overwrite").option("overwriteSchema", "true") \
        .saveAsTable(target_table)

    row_count = scored.count()
    log_run(spark, cfg, job_name="gold_cart_recovery", layer="gold", target_table=target_table,
            row_count=row_count, started_at=started_at)
