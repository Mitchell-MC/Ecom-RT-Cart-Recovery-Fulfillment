...
def open_orders_with_shipment(orders: DataFrame, shipments: DataFrame) -> DataFrame:
    return (
        ...
        .filter(F.col("promised_delivery_date").isNotNull())
        .join(shipments.select("order_id", "carrier", "carrier_avg_transit_days",
                                "carrier_historical_late_rate_pct"),
              on="order_id", how="inner")
    )


def with_inventory_risk(orders: DataFrame, order_items: DataFrame, inventory: DataFrame) -> DataFrame:
    backorder_by_order = (
        order_items.join(inventory.select("sku", "backorder_flag"), on="sku", how="left")
        .groupBy("order_id")
        .agg(F.max(F.coalesce(F.col("backorder_flag"), F.lit(False))).alias("has_backordered_item"))
    )
    return orders.join(backorder_by_order, on="order_id", how="left") \
        .withColumn("has_backordered_item", F.coalesce(F.col("has_backordered_item"), F.lit(False)))


def score(orders: DataFrame) -> DataFrame:
    return (
        orders
        .withColumn("days_to_promise", F.datediff(F.col("promised_delivery_date"), F.current_date()))
        .withColumn("time_pressure_score",
                    100 * (1 - F.least(
                        F.greatest(F.col("days_to_promise"), F.lit(0))
                        / F.col("carrier_avg_transit_days"), F.lit(1.0))))
        .withColumn("inventory_risk_score", F.when(F.col("has_backordered_item"), 100.0).otherwise(0.0))
        .withColumnRenamed("carrier_historical_late_rate_pct", "carrier_risk_score")
        .withColumn("fulfillment_risk_score",
        ...
        .select("order_id", "customer_id", "order_total_usd", "carrier",
...
def main():
    ...
    log_run(spark, cfg, job_name="gold_fulfillment_risk", layer="gold", target_table=target_table,
