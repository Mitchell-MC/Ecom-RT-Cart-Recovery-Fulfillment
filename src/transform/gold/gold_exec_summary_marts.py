...
def recoverable_revenue(cart_recovery_signal: DataFrame) -> float:
    row = cart_recovery_signal.filter(F.col("priority_score") >= PRIORITY_ACTION_THRESHOLD) \
        .agg(F.sum("recoverable_cart_value").alias("total")).collect()[0]
    return row["total"] or 0.0


def delayed_order_revenue_risk(fulfillment_risk_signal: DataFrame) -> float:
    row = fulfillment_risk_signal.filter(F.col("fulfillment_risk_score") >= RISK_ACTION_THRESHOLD) \
        .agg(F.sum("order_total_usd").alias("total")).collect()[0]
    return row["total"] or 0.0
...
def on_time_delivery_rate(shipments: DataFrame) -> float:
    recent = shipments.filter(
        ...
        & (F.col("delivered_at") >= F.expr(f"current_timestamp() - INTERVAL {TRAILING_WINDOW_DAYS} DAYS"))
    ).withColumn("is_on_time", F.to_date("delivered_at") <= F.col("promised_delivery_date"))
...
def conversion_lag_median_minutes(clickstream_events: DataFrame) -> float | None:
    per_cart = (
        clickstream_events
        .filter(F.col("event_type").isin("add_to_cart", "checkout_complete"))
        .filter(F.col("cart_id").isNotNull())
        .groupBy("cart_id")
        .agg(
            F.min(F.when(F.col("event_type") == "add_to_cart", F.col("event_timestamp")))
                .alias("first_add_at"),
            F.max(F.when(F.col("event_type") == "checkout_complete", F.col("event_timestamp")))
                .alias("checkout_complete_at"),
        )
        .filter(F.col("checkout_complete_at").isNotNull())
        .filter(F.col("checkout_complete_at") >=
                F.expr(f"current_timestamp() - INTERVAL {TRAILING_WINDOW_DAYS} DAYS"))
        .withColumn("lag_minutes",
                    (F.unix_timestamp("checkout_complete_at") - F.unix_timestamp("first_add_at")) / 60.0)
    )
    row = per_cart.agg(F.percentile_approx("lag_minutes", 0.5).alias("median")).collect()[0]
    return row["median"]
...
def upsert_daily_row(spark: SparkSession, target_table: str, metrics: dict) -> None:
    ...
    (target.alias("t").merge(row_df.alias("s"), "t.metric_date = s.metric_date")
...
def main():
    ...
    fulfillment_risk_signal = spark.read.table(cfg.table("gold", "fulfillment_risk_signal"))
    ...
    metrics = {
        ...
        "delayed_order_revenue_risk": delayed_order_revenue_risk(fulfillment_risk_signal),
        "on_time_delivery_rate": on_time_delivery_rate(shipments),
        "conversion_lag_median_minutes": conversion_lag_median_minutes(clickstream_events),
    }

    target_table = cfg.table("gold", "exec_summary_daily")
    upsert_daily_row(spark, target_table, metrics)

    log_run(spark, cfg, job_name="gold_exec_summary_marts", layer="gold",
            target_table=target_table, row_count=1, started_at=started_at)
