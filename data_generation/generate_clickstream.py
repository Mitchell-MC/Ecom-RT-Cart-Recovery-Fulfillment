...
CART_ABANDON_AFTER_MIN = (
    30  # must match docs/metric-glossary.md cart_abandon_threshold_minutes
)
...
def inject_bad_row(event: dict, rng: random.Random) -> dict:
    ...
    corruption = rng.choice(
        ["null_price", "negative_price", "future_timestamp", "none"]
    )
...
def generate_session(
    day: datetime, customer, products, rng: random.Random, bad_row_rate: float
):
    session_id = f"SESS-{uuid.uuid4()}"
    device_type = rng.choice(DEVICE_TYPES)
    start = day.replace(
        hour=hour_weight(), minute=rng.randint(0, 59), second=rng.randint(0, 59)
    )
    ts = start
    events = []
    cart_id = None
    ...
    for _ in range(n_pageviews):
        ...
        events.append(
            make_event(
                ...
                cart_id,
                device_type,
                {"page_type": page_type, "sku": product.sku},
            )
        )
...
def main():
    ...
    parser.add_argument(
        "--start-date", default=None, help="YYYY-MM-DD, defaults to today-days"
    )
    ...
    for day_offset in range(args.days):
        ...
        active_today = [
            ...
            for c in customers
            if rng.random()
            < (args.daily_active_rate * (1.6 if c.is_returning else 1.0))
        ]

        for customer in active_today:
            n_sessions = 1 if rng.random() < 0.85 else 2
            for _ in range(n_sessions):
                (events, cart_id, cart_items, converted, checkout_ts, session_id) = (
                    generate_session(day, customer, products, rng, args.bad_row_rate)
                )
        ...
        print(
            f"  {day.date().isoformat()}: {len(day_events)} events, "
            f"{len(active_today)} active customers"
        )

    with (out_root / "_converted_sessions.csv").open(
        "w", newline="", encoding="utf-8"
    ) as f:
        writer = csv.DictWriter(
            f, fieldnames=["cart_id", "customer_id", "session_id", "converted_at"]
        )
        writer.writeheader()
        writer.writerows(converted_sessions)

    with (out_root / "_converted_session_items.csv").open(
        "w", newline="", encoding="utf-8"
    ) as f:
    ...
    print(
        f"Done. {total_events} total events across {args.days} days. "
